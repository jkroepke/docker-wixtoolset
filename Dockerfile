...
# renovate: deb=winehq-stable registry=https://dl.winehq.org/wine-builds/debian/dists/trixie/main/ versioning=regex:^(?<major>\d+)\.(?<minor>\d+)\.(?<patch>\d+)\.(?<build>\d+)~(?<compatibility>.*)-(?<revision>\d+)
